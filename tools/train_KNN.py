...
# -----------------------------------------
# The entire code is to train vision-based safety model using KNN
# -----------------------------------------
...
if not os.path.exists(path + 'safety_estimator/logs/KNN_70_log/'):
    os.mkdir(path + 'safety_estimator/logs/KNN_70_log/')

init_time = str(int(time.time()))
fidout = open(path + 'safety_estimator/logs/KNN_70_log/' + 'log_' + init_time + '.txt', 'a')

# -----------------------------------------
# load matrix for training data (classA + classB)
# -----------------------------------------
X_train = np.load(path + 'safety_estimator/X_train_70.npy')
Y_train = np.load(path + 'safety_estimator/Y_train_70.npy')
...
# save trained model
# -----------------------------------------
with open(path + 'models/pretrained_safety_model_town05_KNN_70.pkl','wb') as f:
    pickle.dump(neigh,f)
...
# load trained model
# -----------------------------------------
with open(path + 'models/pretrained_safety_model_town05_KNN_70.pkl', 'rb') as f:
    neigh = pickle.load(f)
...
print('accuracy of KNN_70: {:0.2f}%'.format(100 * (TP + TN) / (TP + TN + FP + FN)))
print('precision of KNN_70: {:0.2f}%'.format(100 * (TP) / (TP + FP)))
print('recall of KNN_70: {:0.2f}%'.format(100 * (TP) / (TP + FN)))

fidout.write('TP:{} TN:{} FP:{} FN:{}\n'.format(TP, TN, FP, FN))
fidout.write('accuracy of KNN_70: {:0.2f}%\n'.format(100 * (TP + TN) / (TP + TN + FP + FN)))
fidout.write('precision of KNN_70: {:0.2f}%\n'.format(100 * (TP) / (TP + FP)))
fidout.write('recall of KNN_70: {:0.2f}%\n'.format(100 * (TP) / (TP + FN)))