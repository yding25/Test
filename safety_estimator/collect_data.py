...
class SupervisedEncoder:
    def __init__(self):
        ...
        path_to_encoder_dict = path + 'models/model_epoch_34.pth'
    ...
    def run_step(self, image, name, snapshot, save_flag=False):
        ...
        if save_flag:
            for indice_image in self.steps_image:
                ...
                new_bgr = new_image[:, :, ::-1]
                cv2.imwrite(
                    path + 'safety_estimator/raw_data/raw_png/' + str(init_time) + '_' + snapshot + "_" + str(
...
#         - recording
# -----------------------------------------
global path, signal_stop, signal_collision, signal_merge, signal_error, init_time

path = utils.root_path()

init_time = int(time.time())

if not os.path.exists(path + 'safety_estimator/'):
    os.mkdir(path + 'safety_estimator/')
if not os.path.exists(path + 'safety_estimator/raw_data/'):
    os.mkdir(path + 'safety_estimator/raw_data/')
if not os.path.exists(path + 'safety_estimator/raw_data/raw_frame/'):
    os.mkdir(path + 'safety_estimator/raw_data/raw_frame/')
if not os.path.exists(path + 'safety_estimator/raw_data/raw_png/'):
    os.mkdir(path + 'safety_estimator/raw_data/raw_png/')
if not os.path.exists(path + 'safety_estimator/raw_data/recording/'):
    os.mkdir(path + 'safety_estimator/raw_data/recording/')
    
# -----------------------------------------
# this folder is to record each trial's information
# -----------------------------------------
fidin = open(path + 'safety_estimator/raw_data/overview.txt', 'a')
...
def game_loop(args):
    ...
    # -----------------------------------------
    params = {
        'target_lane': 106, # target lane id
    ...
    try:
        ...
        # -----------------------------------------
        recording = path + 'safety_estimator/raw_data/recording/' + str(init_time) + '.log'
        client.start_recorder(recording)

        # -----------------------------------------
        # compute source and destination navigation goals
        # -----------------------------------------
        source_state, dest_state = generate_navigation(path, target_lane)
        ...
        with CarlaSyncMode(world, camera, *cameras_288, fps=10) as sync_mode:
            while (not agent_ego.done()) and (not signal_stop) and (not signal_collision) and (not signal_merge):
                ...
                # -----------------------------------------
                for _ in range(3):  # wait a few frames
                ...
                if save_flag_value:
                    ...
                    velocity = car_ego.get_velocity()
                    acceleration = car_ego.get_acceleration()
                    
                    if math.sqrt(velocity.x ** 2 + velocity.y ** 2) > 0.01:
                        signal_speed = True
                        print('-'*30)
                        print('x:{}, y:{}'.format(velocity.x ** 2, velocity.y ** 2))
                        print('speed:{}'.format(math.sqrt(velocity.x ** 2 + velocity.y ** 2)))
                    else:
                        print('!'*30)
                        print('x:{}, y:{}'.format(velocity.x ** 2, velocity.y ** 2))
                        print('speed:{}'.format(math.sqrt(velocity.x ** 2 + velocity.y ** 2)))
                    ...
                    if signal_speed:
                        front = supervised_encoder_front.run_step(images[0], "Front", str(snapshot), save_flag=save_flag_value)
                        back = supervised_encoder_back.run_step(images[1], "Back", str(snapshot), save_flag=save_flag_value)
                        right = supervised_encoder_right.run_step(images[2], "Right", str(snapshot), save_flag=save_flag_value)
                        left = supervised_encoder_left.run_step(images[3], "Left", str(snapshot), save_flag=save_flag_value)
                    
                        frames_data = np.concatenate((front, back, right, left, (velocity.x, velocity.y, velocity.z, acceleration.x, acceleration.y, acceleration.z)))
                ...
                # -----------------------------------------
                # save frame into folder (5th frame)
                # -----------------------------------------
                if index_frame == params['index_frame'] and signal_speed:
                    np.save(path + 'safety_estimator/raw_data/raw_frame/' + str(init_time) + '_' + str(snapshot), frames_data)
                index_frame += 1
            ...
            # -----------------------------------------
            if signal_speed:
                print('speed is satisfied.')
                if not signal_collision:
                    print('no collision is recorded.')
                    fidin.write('no collision: %s\n' % str(init_time))
                else:
                    print('exist collision is recorded.')
